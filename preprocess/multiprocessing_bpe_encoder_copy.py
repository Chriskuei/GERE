    ...
    def encode_docs(self, lines):
        ...
        for line, doc_lines in zip(lines, doc_lines):
            ...
            for evidence in evidences:
                ...
                map_num = {int(item[0]): idx for idx, item in enumerate(evidence_texts) if item[0] != ''}
                if title not in title_to_id:
                    title_id += 1
                    title_to_id[title] = title_id
                doc_text = {idx: self.encode(item[1]) for idx, item in enumerate(evidence_texts) if item[0] != ''}
                evidence_dict[title_to_id[title]]['doc'] = doc_text
                evidence_dict[title_to_id[title]]['evidence_id'].append(map_num[index])
