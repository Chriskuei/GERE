...
class ConnectionHubInterface(GeneratorHubInterface):
    ...
    #     res = []
    #     for batch in self._build_evidence_batches(tokenized_sentences, skip_invalid_size_inputs):
    ...
    #         res.extend(results)
    #     return res
    def generate(self, *args, retrieval_evidence=False, tokenized_documents=[], **kwargs) -> List[List[Dict[str, torch.Tensor]]]:
        if retrieval_evidence:
            return self.super_generate(*args, **kwargs, tokenized_documents=tokenized_documents)
    ...
    def _build_evidence_batches(
        self, tokens: List[List[int]], docs, skip_invalid_size_inputs: bool
    ) -> Iterator[Dict[str, Any]]:
        lengths = torch.LongTensor([t.numel() for t in tokens])
        batch_iterator = self.task.get_batch_iterator(
            dataset=self.task.build_dataset_for_evidence_inference(tokens, lengths, docs=docs),
    ...
    def super_generate(
        ...
        **kwargs
        ...
        inference_step_args = inference_step_args or {}
        results = []
        for batch in self._build_evidence_batches(tokenized_sentences, tokenized_documents, skip_invalid_size_inputs):
