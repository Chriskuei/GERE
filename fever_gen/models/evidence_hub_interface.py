...
class EvidenceHubInterface(GeneratorHubInterface):
    ...
    def generate(
        ...
        tokenized_documents: List[List[torch.LongTensor]],
        *args,
        ...
        **kwargs
        ...
        res = []
        for batch in self._build_batches(tokenized_sentences, tokenized_documents, skip_invalid_size_inputs):
            src_tokens = batch['net_input']['src_tokens']
            docs_tokens = batch['net_input']['docs_tokens']
    ...
    def sample(
        ...
        **kwargs,
        ...
        # print('**', [hypo["tokens"] for hypos in batched_hypos])
        outputs = [
            [
    ...
    def _build_batches(
        ...
        batch_iterator = self.task.get_batch_iterator(
            dataset=self.task.build_dataset_for_inference(tokens, lengths, docs=docs),
    ...
    def super_generate(
        self,
        tokenized_sentences: List[torch.LongTensor],
        tokenized_documents: List[torch.LongTensor],
        beam: int = 5,
        verbose: bool = False,
        skip_invalid_size_inputs=False,
        inference_step_args=None,
