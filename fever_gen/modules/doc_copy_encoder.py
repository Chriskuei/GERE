...
class DocumentCopyEncoder(FairseqEncoder):
    """
    ...
    def forward_scriptable(
        ...
        sen_num = docs_tokens['sen_num']
        tgt_len = docs_tokens['tgt_len']
        batch_size = docs_tokens['batch_size']
        evidence_mask = docs_tokens['evidence_mask']
        eid_target = docs_tokens['eid_target']
        # dtype = docs_tokens['doc'][0]['1']['doc'].dtype
        try:
            device = docs_tokens['doc'][0]['1']['docs_tokens'].device
        except:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        doc_padding_mask = torch.zeros(batch_size, tgt_len, sen_num, dtype=torch.bool, device=device)
        batch_doc = torch.ones(batch_size, tgt_len, sen_num, 1024, dtype=torch.float16, device=device)
        batch_evidence = torch.zeros(batch_size, tgt_len, 1024, dtype=torch.float16, device=device)
        # print('batch_doc', batch_doc.shape)
        encoder_embedding = torch.zeros(batch_size, tgt_len, sen_num, 1024, dtype=torch.float16, device=device)
        encoder_states = []
        for sid, sample in enumerate(docs_tokens['doc']):
            for d in sample.values():
                ...
                x = x.mean(dim=0)
                # print('***x', x.shape)
                for position, idx in zip(d['position'], d['evidence_id']):
                    batch_doc[sid, position, :x.size(0), :] = x
                    if idx == -1:
                        batch_evidence[sid, position, :] = x[0]
                    else:
                        batch_evidence[sid, position+1, :] = x[idx]
                    doc_padding_mask[sid, position, x.size(0):] = True
        ...
        return {
            ...
            "evidence_input_vector": [batch_evidence],
