...
@register_criterion(
    "fever_copy_label_smoothed_cross_entropy", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class LabelSmoothedCopyCrossEntropyCriterion(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss, e_loss, e_nll_loss = self.compute_loss(model, net_output, sample, reduce=reduce)
        sample_size = (
            sample["target"].size(0) if self.sentence_avg else sample["ntokens"]
        )
        # print(loss, nll_loss, e_loss, e_nll_loss)
        loss = loss + e_loss
        nll_loss = nll_loss + e_nll_loss
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        e_loss, e_nll_loss = label_smoothed_nll_loss(
        ...
        return loss, nll_loss, e_loss, e_nll_loss
