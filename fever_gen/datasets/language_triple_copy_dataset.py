...
def collate(
    ...
    if samples[0].get("doc", None) is not None:
        ...
        # [B, T, S, C]
        sentence_size = max(int(v) for sample in samples for d in sample['doc'].values() for v in d['doc'].keys()) + 2
        
        evidence_mask = torch.zeros_like(target, dtype=torch.bool)
        eid_target = torch.full_like(target, sentence_size-1, dtype=torch.long)
        for sid, sample in enumerate(samples):
            for k, d in sample['doc'].items():
                for position, eid in zip(d['position'], d['evidence_id']):
                    evidence_mask[sid, int(position)] = True
                    eid_target[sid, int(position)] = eid
    ...
    eid_target = eid_target.index_select(0, sort_order)
    # print("evidence_mask", evidence_mask)
    # print("eid_target", eid_target)
...
class LanguageTripleCopyDataset(FairseqDataset):
    """
