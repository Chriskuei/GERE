...
def collate(
    ...
    ### Document representation: [B, T, S]
    # if samples[0].get("doc", None) is not None:
    #     # print('samples', samples)
    #     size = max(v.size(0) for sample in samples for d in sample['doc'].values() for v in d['doc'].values())
    #     sentence_size = max(int(v) for sample in samples for d in sample['doc'].values() for v in d['doc'].keys()) + 1
    #     # sentence_size = max(len(d['doc']) for sample in samples for d in sample['doc'].values())
    #     size = size if pad_to_length is None else max(size, pad_to_length["doc"])
    #     docs_tokens = torch.full((target.shape[0], target.shape[1], sentence_size, size), pad_idx, dtype=torch.long)
    #     # print('docs_tokens', docs_tokens.shape)
    #     # [B, T, S, C]
    #     for sid, sample in enumerate(samples):
    #         # print('sample doc', sample['doc'])
    #         # print('sample', sample)
    #         for d in sample['doc'].values():
    #             for position in d['position']:
    #                 for k, v in d['doc'].items():
    #                     docs_tokens[sid, position, int(k), :v.size(-1)] = v[:]
    #         # for d in sample['doc'].values():
    #         #     for position in d['position']:
    #         #         for k, v in enumerate(d['doc'].values()):
    #         #             docs_tokens[sid, position, k, :v.size(-1)] = v[:]
    #     docs_tokens = docs_tokens.index_select(0, sort_order)
    # print('####process doc')
    if samples[0].get("doc", None) is not None:
        # print('samples', samples)
        # [B, T, S, C]
        for sid, sample in enumerate(samples):
            ...
            for k, d in sample['doc'].items():
                ...
                #     [torch.LongTensor(s).ne(pad_idx).long().sum() for s in d['doc'].values()]
                # )
                docs_lengths = torch.LongTensor(
                    [torch.LongTensor(s).ne(pad_idx).long().sum() for s in d['doc'].values()]
                )
                ...
                samples[sid]['doc'][k]['docs_tokens'] = docs_tokens
            # for d in sample['doc'].values():
            #     for position in d['position']:
            #         for k, v in enumerate(d['doc'].values()):
            #             docs_tokens[sid, position, k, :v.size(-1)] = v[:]
    ...
    batch = {
        ...
        "ntokens": ntokens,
        "net_input": {"src_tokens": src_tokens, "src_lengths": src_lengths, "docs_tokens": docs_tokens},
        "target": target,
    }
