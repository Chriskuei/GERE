...
class DummyTrieEvidence(object):
    ...
    def get(self, indices, depth=0):
        if len(indices) == 0 and depth == 0:
            return self._codes["end_title_token"]
        elif len(indices) == 0 and depth == 1:
            return self._codes["start_evidence_token"]
        elif len(indices) == 0:
            return self._return_values
        elif len(indices) == 1 and indices[0] == self._codes["end_evidence_token"]:
            return self._codes["EOS"]
        else:
            return self.get(indices[1:], depth=depth + 1)

# [ title1 ] { evidence1 } { evidence2 } [ title2 ] { evidence2 }
class DummyTrieContinueEvidence(object):
    def __init__(self, return_values, codes):
        self._return_values = list(
            set(return_values).difference(
                set(
