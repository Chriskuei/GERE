...
class PrefixDocumentConstrainedBeamSearch(Search):
    ...
    def apply_mask(self, x, prev_output_tokens, original_batch_idxs):
        ...
        for sent_i, (sent, batch_i) in enumerate(
            ...
            available_ids, document = self.prefix_allowed_tokens_fn(batch_i, sent)
            mask[sent_i, :, available_ids] = 0
    ...
    def step(
        ...
        mask, documents = self.apply_mask(
            lprobs.view(bsz * beam_size, 1, vocab_size),
            prev_output_tokens,
            original_batch_idxs,
        )
        lprobs += mask.view(bsz, beam_size, vocab_size)
        ...
        beams_buf = indices_buf // vocab_size
        indices_buf = indices_buf.fmod(vocab_size)
        return scores_buf, indices_buf, beams_buf, documents
