...
def make_builder(out_file, impl, vocab_size=None):
    if impl == "mmap":
        ...
        return MMapJsonIndexedDatasetBuilder(
            out_file, dtype=best_fitting_int_dtype(vocab_size)
        )
    elif impl == "fasta":
...
def make_dataset(path, impl, fix_lua_indexing=False, dictionary=None):
    if impl == "json" and IndexedRawJSONDataset.exists(path):
        return IndexedRawJSONDataset(path, dictionary)
    elif impl == "mmap_json" and MMapJsonIndexedDataset.exists(path):
        return MMapJsonIndexedDataset(path)
...
class IndexedRawJSONDataset(FairseqDataset):
    """Takes a json file as input and binarizes it in memory at instantiation.
    Original lines are also kept in memory"""
...
class MMapJsonIndexedDataset(torch.utils.data.Dataset):
    class Index:
        _HDR_MAGIC = b"MMIDIDX\x00\x00"
    ...
    def __getitem__(self, i):
        ptr, size = self._index[i]
        np_array = np.frombuffer(
            self._bin_buffer, dtype=f'<U{size}', count=1, offset=ptr
        ).tolist()[0]

        return json.loads(np_array)
...
class MMapJsonIndexedDatasetBuilder:
    def __init__(self, out_file, dtype=np.int64):
        self._data_file = open(out_file, "wb")
        self._dtype = np.str
        self._sizes = []
    ...
    def add_item(self, line):
        ...
        np_array = np.array(line)
        byte = np_array.tobytes(order="C")
        self._data_file.write(byte)
        self._sizes.append(len(line))

    def merge_file_(self, another_file):
        # Concatenate index
        index = MMapJsonIndexedDataset.Index(index_file_path(another_file))
        assert index.dtype == self._dtype
    ...
    def finalize(self, index_file):
        self._data_file.close()
        with MMapJsonIndexedDataset.Index.writer(index_file, self._dtype) as index:
            index.write(self._sizes)
