...
class ModelCriterion(FairseqCriterion):
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        e_loss_sum = utils.item(sum(log.get("e_loss", 0) for log in logging_outputs))
        ...
        e_sample_size = utils.item(
            sum(log.get("e_sample_size", 1) for log in logging_outputs)
        )

        metrics.log_scalar("loss", loss_sum / sample_size, sample_size, round=3)
        metrics.log_scalar("e_loss", e_loss_sum / e_sample_size, e_sample_size, round=3)
